...
ui <- fluidPage(
    navbarPage(title = "Co-mutation check with AACR-GENIE v11.0 data",theme = shinytheme("paper"),
               ...
               tabPanel(title = "Adhoc analysis for a specific gene pair",
                        ...
                        fluidRow(
                            useShinyjs(),
                            withSpinner(tagList(DT::dataTableOutput('dt'),plotlyOutput(outputId = 'scPlot',height = '900px')))
                        )
               ))
)
...
server <- function(input, output,session) {
    # screen results
    output$`gp-dt`<-DT::renderDataTable(DT::datatable(result.list[[ifelse(input$screen.w.CNA=='yes','w','wo')]][['comut']],options = list(pageLength=5,scrollX = TRUE),filter='top',rownames = F,container = sketch.gp))
    output$`type-dt`<-DT::renderDataTable(DT::datatable(result.list[[ifelse(input$screen.w.CNA=='yes','w','wo')]][['type']],options = list(pageLength=5,scrollX = TRUE),filter='top',rownames = F,container = sketch.type))
    output$`full-dt`<-DT::renderDataTable(DT::datatable(result.list[[ifelse(input$screen.w.CNA=='yes','w','wo')]][['full']],options = list(pageLength=5),filter='top',rownames = F,container = sketch.full))
    ...
    observeEvent(input$submit,{
        if(input$g1==input$g2) showNotification("Please choose two different genes.")else{
            output$dt<-DT::renderDataTable(DT::datatable({isolate(co.df())},options = list(pageLength=10),filter='top',rownames = F,container = sketch.adhoc))
